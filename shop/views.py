...
def index(request):
    ...
    clean_categories = [c for c in CATEGORIES if "bowl" not in c["name"].lower() and "bowl" not in c["slug"].lower()]
...
def category_detail(request, category_slug):
    ...
    if category_slug == "phones":
        category_products = [p for p in category_products if "bowl" not in p["name"].lower() and "bowl" not in str(p["image"]).lower()]
